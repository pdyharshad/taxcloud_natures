...
class TestSaleAccountTaxCloud(TestAccountTaxcloudCommon):
    def test_01_taxcloud_full_flow(self):
        ...
        # This tax amount depends on many factors like
        # the category of the product and the address of the customer.
        # When we receive a response from TaxCloud,
        # we compute the tax rate with this formula: (tax_amount / price * 100)
        # If no tax exists with that tax rate, the tax is created. Otherwise, it is reused.
        # Depending on the address and the price,
        # it is possible that the computation of the tax rate for the 2 lines
        # is slightly different (like 8.225% for the first line and 8.230%
        ...
        self.assertEqual(payment.amount, 1007.69)
